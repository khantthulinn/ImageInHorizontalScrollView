...
@interface MWPhotoBrowser() <scrollViewDidZoomDelegate>
...
- (void)viewDidLoad {
    ...
    _pagingScrollView.delaysContentTouches = YES;
    _pagingScrollView.canCancelContentTouches = YES;
//    _pagingScrollView.scrollEnabled = NO;
//    _pagingScrollView.panGestureRecognizer.delegate = self;
...
//            page.delegate = self;
//            [_pagingScrollView.panGestureRecognizer requireGestureRecognizerToFail:page.panGestureRecognizer];
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
    ...
    for (MWZoomingScrollView *page in _visiblePages)
    {
        //TODO: if zoom and content is showing rightest side of page, then go to next page..
        
        if (page.zoomScale < 1.0)
        {
            page.panGestureRecognizer.enabled = NO;
        }
        else
        {
            NSInteger scrollviewWidth = page.contentSize.width - page.contentOffset.x;
//            NSInteger limit = 10;
//            NSInteger lowerBoundary = CGRectGetWidth(page.frame) - limit;
            if (scrollviewWidth == CGRectGetWidth(page.frame))
//            if (scrollviewWidth > lowerBoundary  && scrollviewWidth <= CGRectGetWidth(page.frame))
            {
                page.panGestureRecognizer.enabled = NO;
            }
            else
            {
                page.panGestureRecognizer.enabled = YES;
                [_pagingScrollView.panGestureRecognizer requireGestureRecognizerToFail:page.panGestureRecognizer];
            }
        }
    }
...
#pragma mark - scrollViewDidZoomDelegate

- (void)scrollViewDidZoomFromSubView:(UIScrollView *)scrollView
{
    if (scrollView.zoomScale == 1.0)
    {
        [_pagingScrollView.panGestureRecognizer requireGestureRecognizerToFail:nil];
    }
    else
    {
        [_pagingScrollView.panGestureRecognizer requireGestureRecognizerToFail:scrollView.panGestureRecognizer];
    }
}
...
- (void)swipeDown:(id)sender
{
    for (MWZoomingScrollView *page in _visiblePages)
    {
        //visible page will be only 1
        self.currentZoomScale = page.zoomScale;
    }
    
    if (self.currentZoomScale > 1) return;
